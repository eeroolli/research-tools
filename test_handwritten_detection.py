#!/usr/bin/env python3
"""
Test script for handwritten note detection.
Tests the detection with the example file.
"""
...
def test_handwritten_detection():
    """Test handwritten note detection with example file."""
    ...
    is_handwritten = processor._check_if_handwritten_note(pdf_path, page_offset=0, max_pages_to_check=2)
    
    print(f"\n📊 Detection Result:")
    print(f"   Detected as handwritten note: {is_handwritten}")
    
    # Check threshold
    threshold = processor._read_handwritten_threshold_from_config()
