#!/usr/bin/env python3
"""Test script for gutter detection on a specific PDF."""
...
def normalize_path_for_wsl(path_str: str) -> str:
    ...
    if ':' in path_str or (len(path_str) > 1 and path_str[1].isalpha() and path_str[1] != ':'):
        # Handle Windows paths like "I:\path\to\file" or "I:/path/to/file"
        path_str = path_str.replace('\\', '/')
...
def test_gutter_detection(pdf_path: str):
    """Test gutter detection on a specific PDF."""
    # Normalize path (convert Windows paths to WSL format if needed)
    normalized_path = normalize_path_for_wsl(pdf_path)
    ...
    # Visualize projection profiles
    print("\n2. Analyzing projection profiles...")
    try:
        ...
        if spine_signal > 0.15:
            print(f"     → Using SPINE method (physical book)")
            final_pdf = spine_pdf
            final_pct = spine_pct
        else:
            print(f"     → Using CONTENT method (printed article)")
            final_pdf = content_pdf
            final_pct = content_pct
        
        print(f"\n   Final gutter: {final_pdf:.1f} points ({final_pct:.1f}%)")
        print(f"\n📊 Summary:")
...
if __name__ == "__main__":
    ...
    # Default test files (Windows format - will be normalized)
    default_files = [
        r"I:\FraScanner\papers\done\EN_20260103_0002_double.pdf",
        r"I:\FraScanner\papers\done\EN_20260102_0001_double.pdf",
        r"I:\FraScanner\papers\done\EN_20260102_0002_double.pdf",
    ]
    
    if len(sys.argv) > 1:
        # Use files from command line (normalize each one)
        pdf_files = [normalize_path_for_wsl(arg) for arg in sys.argv[1:]]
    else:
        # Use default files (normalize each one)
        pdf_files = [normalize_path_for_wsl(f) for f in default_files]
    
    try:
        for i, pdf_path_str in enumerate(pdf_files, 1):
            if len(pdf_files) > 1:
                print(f"\n{'='*70}")
                print(f"FILE {i}/{len(pdf_files)}: {Path(pdf_path_str).name}")
                print(f"{'='*70}")
            test_gutter_detection(pdf_path_str)
            if i < len(pdf_files):
                print("\n")  # Add spacing between files
    except ImportError as e:
        print(f"❌ Missing dependency: {e}")
        print("\nPlease install required packages:")
        print("  pip install PyMuPDF numpy opencv-python")
        sys.exit(1)
    except Exception as e:
        print(f"❌ Error: {e}")
        import traceback
